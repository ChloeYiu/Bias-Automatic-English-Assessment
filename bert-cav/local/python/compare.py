...
def main(args):
    ...
    for name in graph_list:
        if name == 'Unbiased':
            top_name = f"{top_dir}/{train_set}/bias/{train_set}/{bias_set}/part{part}/{feature}"
            pred_dir = f"{top_dir}/{train_set}/predictions/{bias_set}/part{part}"
        else:
            top_name = f"{top_dir}/{train_set}_{bias_model}/bias/{train_set}_{bias_model}/{bias_set}/part{part}/{feature}"
            pred_dir = f"{top_dir}/{train_set}_{bias_model}/predictions/{bias_set}/part{part}"
        for seed in bias_multiple.seed_list:
            if class_weight:
                distance_file = f"{top_name}/bias_bert_part{part}_{seed}_layer{layer}_{class_weight}.txt"
            else:
                distance_file = f"{top_name}/bias_bert_part{part}_{seed}_layer{layer}.txt"            
            pred_file = f"{pred_dir}/preds_bert_part{part}_{seed}.txt"
