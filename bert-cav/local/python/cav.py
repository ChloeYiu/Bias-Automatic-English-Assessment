...
class FileGenerator:
    ...
    def __init__(self, model_name, dir, file_type):
        self.dir = dir
        self.file_prefix = f'{file_type}_{model_name}_layer'

    def tmp_file(self, layer_num):
        return os.path.join(self.dir, f'{self.file_prefix}{layer_num}{self.tmp_suffix}')

    def write_tmp_file(self, layer_num, data):
        with open(self.tmp_file(layer_num), 'a') as f:
            f.write(f'{data}\n')
    
    def read_tmp_file(self, layer_num, f):
        return f.readline().strip()

    def file(self, layer_num):
        return os.path.join(self.dir, f'{self.file_prefix}{layer_num}{self.suffix}')

    def filtered_file(self, layer_num):
        return os.path.join(self.dir, f'{self.file_prefix}{layer_num}{self.filtered_suffix}')

    def create_dir(self, layer_nums):
        os.makedirs(self.dir, exist_ok=True)

        for layer_num in layer_nums:
            if os.path.exists(self.tmp_file(layer_num)):
                os.remove(self.tmp_file(layer_num))
            open(self.file(layer_num), 'w').close() 

class ActivationGetter:
    def __init__(self, model, model_name, activation_dir, gradient_dir, layer_nums, batch_size=1):
        self.model = model
        ...
        self.activation_file_generator.create_dir(self.layer_nums)
        self.gradient_file_generator.create_dir(self.layer_nums)

    def add_hooks(self):
        for layer_num in self.layer_nums:
            layer_name = f'layer{layer_num}'
            layer = self.model._modules.get(layer_name)
            partial_hook_fn = partial(self.hook_fn, layer_num=layer_num)
            hook = layer.register_forward_hook(partial_hook_fn)
            self.hooks.append(hook)
            print(f"Hook added to layer {layer_num}")
    ...
    def hook_fn(self, module, input, outputs, layer_num):
        self.activation_cache[layer_num] = outputs
        # Append the tensor with a separator
        for output in outputs.clone().detach().cpu().requires_grad_(True).tolist():
            self.activation_file_generator.write_tmp_file(layer_num, output)

    def store_activations(self, speakerids):
        for layer_num in self.layer_nums:
            activation_tmp_file = self.activation_file_generator.tmp_file(layer_num)
            activation_file = self.activation_file_generator.file(layer_num)
            ...
            with open(activation_tmp_file, 'r') as t:
                while line_counter < len(speakerids):
                    try:
                        activation = self.activation_file_generator.read_tmp_file(layer_num, t)
            ...
            print(f"Total activations stored for layer {layer_num}: {line_counter}")
    ...
    def store_tmp_gradient(self, gradients):
        for i, layer_num in enumerate(self.layer_nums):
            ...
            for gradient in gradients_layer:
                self.gradient_file_generator.write_tmp_file(layer_num, gradient)

    def store_gradients(self, speakerids):
        for layer_num in self.layer_nums:
            gradient_tmp_file = self.gradient_file_generator.tmp_file(layer_num)
            gradient_file = self.gradient_file_generator.file(layer_num)
            ...
            with open(gradient_tmp_file, 'r') as t:
                while line_counter < len(speakerids):
                    try:
                        gradient = self.gradient_file_generator.read_tmp_file(layer_num, t)
                        speakerid = speakerids[line_counter]
            ...
            print(f"Total gradients stored for layer {layer_num}: {line_counter}")
