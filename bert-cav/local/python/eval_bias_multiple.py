...
def main(args):
    ...
    if class_weight:
        plot_file_multiple = f"{top_dir}/{train_set}/bias/{cav_set}/{bias_set}/part{part}/{bias}/bias_bert_part{part}_layer{layer}_{class_weight}.png"
        plot_file_avg = f"{top_dir}/{train_set}/bias/{cav_set}/{bias_set}/part{part}/{bias}/bias_bert_part{part}_layer{layer}_avg_{class_weight}.png"
    else:
        plot_file_multiple = f"{top_dir}/{train_set}/bias/{cav_set}/{bias_set}/part{part}/{bias}/bias_bert_part{part}_layer{layer}.png"
        plot_file_avg = f"{top_dir}/{train_set}/bias/{cav_set}/{bias_set}/part{part}/{bias}/bias_bert_part{part}_layer{layer}_avg.png"

    for seed in bias_multiple.seed_list:
        if class_weight:
            cav_file = f"{top_dir}/{train_set}/cav/{cav_set}/part{part}/{bias}/cav_bert_part{part}_{seed}_layer{layer}_{class_weight}.txt"
            distance_file = f"{top_dir}/{train_set}/bias/{cav_set}/{bias_set}/part{part}/{bias}/bias_bert_part{part}_{seed}_layer{layer}_{class_weight}.txt"
        else:
