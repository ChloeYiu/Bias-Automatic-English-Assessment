...
from ddn_model import LogToFileCallback
from ddn_mt_model import LitModel
...
def DDN_trainer(cfg):
    ...
    working_root_dir = Path('DDN-MT', *train_data.parts[1:])
    working_dir = os.path.join(working_root_dir,'DDN-MT_'+ str(cfg.grader_seed))
    ...
    if (cfg.ckpt_path is None):
        if os.path.isfile(os.path.join(working_dir,'last.ckpt')):
            ckpt_path=os.path.join(working_dir,'last.ckpt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_decay', type=float, default=4e-7, help='Weight decay for the optimizer')
    parser.add_argument('--lr_decay', type=float, default=1, help='Learning rate decay factor')



    ## Additional arguments for DDN-MT
    parser.add_argument('--fa_model_path', type=str, required=True, help='Path to the Fa model file')
    parser.add_argument('--fa_sigma', type=float, default=1.3, help='Sigma value for Fa model')
    parser.add_argument('--fa_alpha', type=float, default=1, help='alpha value for Fa model')
    parser.add_argument('--fa_dsigma', type=float, default=6.0, help='Delta sigma value for Fa model')
    parser.add_argument('--fa_dbias', type=float, default=15, help='Bias value for Fa model')
    parser.add_argument('--epsilon', type=float, default=1e-08, help='Epsilon value for Fa model')
    parser.add_argument('--fa_beta', type=float, default=1, help='Beta value for Fa model')
    parser.add_argument('--reverse_kl', default=True, help='Flag to enable reverse KL (default is False)')
    parser.add_argument('--var_lim', type=float, default=2, help='Value to clamp the variance')
