...
def main(args):
    ...
    for name in graph_list:
        if name == 'Unbiased':
            top_name = f"{top_dir}/{train_set}/bias/{train_set}/{feature}/{bias_set}"
            pred_dir = f"{top_dir}/{train_set}/f4-ppl-c2-pdf/part{part}"
        else:
            top_name = f"{top_dir}/{train_set}_{bias_model}/bias/{train_set}_{bias_model}/{feature}/{bias_set}"
            pred_dir = f"{top_dir}/{train_set}_{bias_model}/f4-ppl-c2-pdf/part{part}"
        for seed in bias_multiple.seed_list:
            ...
            pred_file = f"{pred_dir}/{seed}/{bias_set}/{bias_set}_pred_ref.txt"
