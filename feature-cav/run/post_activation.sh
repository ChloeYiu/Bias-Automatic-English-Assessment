...
LOG=Logs/run/post_activation.log
    mkdir -p $(dirname $LOG)
    if [ -f $LOG ]; then
        \rm $LOG
    fi
...
for part in 1; do
  for seed in "${seeds[@]}"; do
    ...
    cmd="python local/training/post_activation.py --model_dir $top_outdir/f4-ppl-c2-pdf/part${part}/${model}_${seed} --GRADIENT_DIR $top_outdir/gradients/$test_data --ACTIVATION_DIR $top_outdir/activations/$test_data --model_type $model"
