...
for part in 1; do
  ...
  for seed in "${seeds[@]}"; do
    if [ -n "$class_weight" ]; then
        input_base_name=$top_outdir/cav/$cavset/$profile/$testset/eval_cav_part${part}_DDN_${seed}_input_layer_$class_weight
    else
        input_base_name=$top_outdir/cav/$cavset/$profile/$testset/eval_cav_part${part}_DDN_${seed}_input_layer
    fi
