...
if [ $# -lt 3 ]; then
  echo "Usage: $0 <trainset> <testset> <file_suffix>"
  exit 1
fi
...
for part in 1; do
  ...
  prediction_file=$top_outdir/predictions/$testset/preds_wav2vec_part${part}_${file_suffix}.txt
  output_base_name=$top_outdir/predictions/$testset/eval_wav2vec_part${part}_${file_suffix}
