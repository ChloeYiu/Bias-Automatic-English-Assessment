...
for part in 1; do
  ...
  for seed in "${seeds[@]}"; do
    activation_base_name=$top_outdir/activations/$cavset/activations_part${part}_seed${seed}_dense

    if [ -n "$class_weight" ]; then
        output_base_name=$top_outdir/cav/$cavset/$profile/cav_part${part}_seed${seed}_dense_$class_weight
    else
        output_base_name=$top_outdir/cav/$cavset/$profile/cav_part${part}_seed${seed}_dense
    fi
